<script lang="ts">
    ...
    function createContentInstance(type: keyof typeToContent): InstanceType<typeToContent[keyof typeToContent]> {
    ...
    const saveTemplate = async (dataHolder:FSRdata[]) => {
        ...
        if($auth_user !== null){
            await addDoc(collection(db, "companies", $auth_user.company, "fsr_templates"), {
                NAME: templateName,
...
<div class="big-container">
    ...
    <input type="text" id="template-name" placeholder="Enter Template Name" bind:value={templateName}/>
    <button disabled={isSaveDisabled} on:click={() => saveTemplate(fsrDataHolder)}>Save Template</button>
    <div class="container">
